<template>
  <div class="skills">
    ...
    <div v-else>
      <div class="d-flex justify-content-center" v-if="skills.length === 0">
        <img src="@/assets/img/empty.svg" alt="" />
      </div>
      <div v-else>
        <el-collapse v-model="activeNames" @change="handleChange">
          <el-collapse-item
            v-for="(domain, key_domain) in skills"
            :key="domain.name"
            :title="domain.name"
            :name="key_domain"
          >
            ...
            <div class="skills-content mt-3">
              <CompetenciesCard
                class="col-md-4"
                v-for="skill in domain.core[competency[key_domain]?.activeCore]?.skills"
                :key="skill.id"
                :cxCompetence="skill"
              />
            </div>
