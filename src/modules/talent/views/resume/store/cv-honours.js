...
export default {
  ...
  actions: {
    ...
    async list({ commit }) {
      ...
      try {
        let res = await $request.get(`talent/resume/awards`);
    ...
    async view({ commit }, id) {
      ...
      commit("SET_LOADING", true);
      try {
        let res = await $request.get(`talent/resume/awards/${id}`);
        console.log(res.data);
        let responsePayload = res.data.data;
    ...
    async create({ commit, dispatch }, payload) {
      commit("SET_LOADING");
      try {
        let response = await $request.post("talent/resume/awards", payload);
    ...
    // Delete Award
    async deleteAward({ commit, dispatch }, id) {
      commit("SET_LOADING");
      try {
        let response = await $request.delete("talent/resume/awards/" + id);
        console.log(response);
        let responsePayload = response.data;
        toastify({
          text: `${responsePayload.message}`,
