...
export default {
  ...
  actions: {
    ...
    async list({ commit }) {
      ...
      try {
        let res = await $request.get(`talent/resume/hobbies`);
    ...
    async view({ commit }, id) {
      ...
      commit("SET_LOADING", true);
      try {
        let res = await $request.get(`talent/resume/hobbies/${id}`);
        console.log(res.data);
        let responsePayload = res.data.data;
    ...
    async create({ commit, dispatch }, payload) {
      commit("SET_LOADING");
      try {
        let response = await $request.post("talent/resume/hobbies", payload);
    ...
    async update({ commit, dispatch }, {id, payload}) {
      commit("SET_LOADING");
      try {
        let response = await $request.put("talent/resume/hobbies/"+id, payload);
        console.log(response);
        let responsePayload = response.data;
        toastify({
          text: `${responsePayload.message}`,
     ...
     // Delete Hobby
     async deleteHobbies({ commit, dispatch }, id) {
      commit("SET_LOADING");
      try {
        let response = await $request.delete("talent/resume/hobbies/"+id);
        console.log(response);
        let responsePayload = response.data;
        toastify({
          text: `${responsePayload.message}`,
